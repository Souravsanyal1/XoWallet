...
🌟 *Premium EVM Multi-Chain Tracker*
▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬
...
🚀 *System Status:*
▫️ **Version:** 2.2.0 (Premium Build)
▫️ **Core Engine:** Node.js / Ethers.js
▫️ **Uptime:** 24/7 (Monitored by Render)

🌐 *Specialized Services:*
▫️ Custom Telegram Bot Development
▫️ Real-time Blockchain Scanner & Alerts
▫️ Smart Contract Analysis & Tracking
▫️ Premium UI/UX Design for Bot

📞 *Support & Inquiry:*
For custom solutions or technical support, feel free to reach out via the profile link above.
        `.trim(),
    ...
    bn: {
        ...
        dev_text: (name, username, id) => `
🌟 *প্রিমিয়াম EVM মাল্টি-চেইন ট্র্যাকার*
▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬
...
🚀 *সিস্টেম স্ট্যাটাস:*
▫️ **ভার্সন:** ২.২.০ (প্রিমিয়াম বিল্ড)
▫️ **কোর ইঞ্জিন:** Node.js / Ethers.js
▫️ **আপটাইম:** ২৪/৭ (Render দ্বারা মনিটরকৃত)

🌐 *বিশেষায়িত সেবাসমূহ:*
▫️ কাস্টম টেলিগ্রাম বট ডেভেলপমেন্ট
▫️ রিয়েল-টাইম ব্লকচেইন স্ক্যানার এবং অ্যালার্ট
▫️ স্মার্ট কন্ট্রাক্ট অ্যানালাইসিস এবং ট্র্যাকিং
▫️ বটের জন্য প্রিমিয়াম UI/UX ডিজাইন

📞 *সাপোর্ট এবং ইনকয়ারি:*
যেকোনো ধরনের কাস্টম সলিউশন বা টেকনিক্যাল সাপোর্টের জন্য উপরের প্রোফাইল লিঙ্কে যোগাযোগ করুন।
        `.trim(),
    ...
    const handleDevInfo = async (ctx) => {
        ...
        if (devCache.isFetching) {
            ...
            const menu = Markup.inlineKeyboard([
                ...
                ...(await getPersistentMenu(ctx)).reply_markup.inline_keyboard
            ]);
        ...
        if (now - devCache.lastFetch < CACHE_TTL && devCache.lastFetch !== 0) {
            ...
            if (devCache.photoId) {
                return ctx.replyWithPhoto(devCache.photoId, { caption: msg, parse_mode: 'Markdown', ...menu });
            }
            return ctx.reply(msg, { parse_mode: 'Markdown', ...menu });
        }
