...
const i18n = {
    en: {
        welcome: (count) => `💎 *Premium EVM Multi-Chain Tracker* 👑\n\n📊 *Total Wallets Connected:* ${count}\n\n✨ *Advanced Management Console:*`,
        add_wallet: '➕ Add Wallet',
        list_wallets: '📋 List Wallets',
        check_balances: '💰 My Balances',
        dashboard: '📈 Dashboard',
        ...
        tracked_wallets: '💎 *Premium Tracked Wallets* ✨',
        ...
        tx_alert_title: (network) => `✨ *${network} Transaction Alert* ✨`,
...
👑 *Premium EVM Tracker Pro*
▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬

👨‍💻 *Developer Profile*
▫️ *Name:* [${name}](tg://user?id=${id})
▫️ *Telegram:* ${username}
▫️ *Role:* Flutter App & Telegram Bot Developer

🚀 *System Information*
▫️ *Version:* v2.3.0 (Elite Premium)
▫️ *Engine:* Node.js • Ethers.js
▫️ *Network:* Multi-Chain EVM Support
▫️ *Status:* Online 24/7

🛠 *Developer Services*
▫️ Telegram Bot Development
▫️ Flutter App Development
▫️ Crypto / Web3 Tools
▫️ Wallet Tracker Systems
▫️ Automation Bots

📩 *Support & Collaboration*
For bot development, Flutter apps, or custom crypto tools, contact:
        `.trim(),
        ...
        delete_btn: '🗑️ Delete',
        tx_hash: 'Tx hash',
        wallet_dashboard: (name) => `💎 *${name} Dashboard*`,
        all_networks: 'BSC-T · FTM · AVAX · LINEA · BNB · MATIC · ETH · ARB · OPBNB · MNT · OP · PLS · ZKS · BASE · BLAST · SCROLL · SONIC · UNI · HYPE-EVM · ABST · INK · BERA · PLASMA · STORY · MON · MEGAETH'
    },
    bn: {
        welcome: (count) => `💎 *প্রিমিয়াম EVM মাল্টি-চেইন ট্র্যাকার* 👑\n\n📊 *মোট সংযুক্ত ওয়ালেট:* ${count}\n\n✨ *অ্যাডভান্সড ম্যানেজমেন্ট কনসোল:*`,
        add_wallet: '➕ ওয়ালেট যোগ',
        list_wallets: '📋 ওয়ালেট লিস্ট',
        check_balances: '💰 আমার ব্যালেন্স',
        ...
        tracked_wallets: '💎 *প্রিমিয়াম ট্র্যাকড ওয়ালেটসমূহ* ✨',
        ...
        tx_alert_title: (network) => `✨ *${network} ট্রানজেকশন অ্যালার্ট* ✨`,
        ...
        dev_info: '👤 Bot Developer',
        dev_text: (name, username, id) => `
👑 *প্রিমিয়াম EVM ট্র্যাকার প্রো*
▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬

👨‍💻 *ডেভেলপার প্রোফাইল*
▫️ *নাম:* [${name}](tg://user?id=${id})
▫️ *টেলিগ্রাম:* ${username}
▫️ *রোল:* ফ্লাটার অ্যাপ এবং টেলিগ্রাম বট ডেভেলপার

🚀 *সিস্টেম ইনফরমেশন*
▫️ *ভার্সন:* v২.৩.০ (এলিট প্রিমিয়াম)
▫️ *ইঞ্জিন:* Node.js • Ethers.js
▫️ *নেটওয়ার্ক:* মাল্টি-চেইন EVM সাপোর্ট
▫️ *স্ট্যাটাস:* অনলাইন ২৪/৭

🛠 *ডেভেলপার সেবাসমূহ*
▫️ টেলিগ্রাম বট ডেভেলপমেন্ট
▫️ ফ্লাটার অ্যাপ ডেভেলপমেন্ট
▫️ ক্রিপ্টো / ওয়েব৩ টুলস
▫️ ওয়ালেট ট্র্যাকার সিস্টেম
▫️ অটোমেশন বোট

📩 *সাপোর্ট এবং কোলাবরেশন*
বট ডেভেলপমেন্ট, ফ্লাটার অ্যাপস বা কাস্টম ক্রিপ্টো টুলের জন্য যোগাযোগ করুন:
        `.trim(),
        ...
        delete_btn: '🗑️ ডিলিট',
        tx_hash: 'Tx hash',
        wallet_dashboard: (name) => `💎 *${name} ড্যাশবোর্ড*`,
        all_networks: 'BSC-T · FTM · AVAX · LINEA · BNB · MATIC · ETH · ARB · OPBNB · MNT · OP · PLS · ZKS · BASE · BLAST · SCROLL · SONIC · UNI · HYPE-EVM · ABST · INK · BERA · PLASMA · STORY · MON · MEGAETH'
    }
};
...
function setupBot(token, wallets, onAddWallet, onRemoveWallet, getUserLanguage, setUserLanguage, onRename, onToggleFavorite) {
    ...
    // Helper: Escape Markdown V1 special characters
    ...
    const getPersistentMenu = async (ctx) => {
        try {
            const add = await getText(ctx, 'add_wallet') || '➕ Add';
            const list = await getText(ctx, 'list_wallets') || '📋 List';
            const bal = await getText(ctx, 'check_balances') || '💰 Balances';
            const dash = await getText(ctx, 'dashboard') || '📊 Dashboard';
            const lang = await getText(ctx, 'lang') || '🌐 Language';
            const help = await getText(ctx, 'help') || 'ℹ️ Help';
            const rem = await getText(ctx, 'remove_wallet') || '🗑️ Remove';
            const dev = await getText(ctx, 'dev_info') || '👤 Developer';

            let kb = Markup.keyboard([
                [add, list, bal],
                [lang, help, dash],
                [rem],
                [dev]
            ...
            // Only use persistent if supported and doesn't crash
            if (typeof kb.persistent === 'function') {
                kb = kb.persistent();
            }
            return kb;
        } catch (e) {
            console.error('Error in getPersistentMenu:', e);
    ...
    const handleHome = async (ctx) => {
        try {
            ...
            const bannerPath = './assets/welcome_banner.png';
            const fs = require('fs');
            ...
            console.error('handleHome Error:', error);
            await ctx.reply('Welcome back! Use /start if the menu is missing.').catch(() => { });
        }
    };
...
\`${wallet.address}\`
    ...
    const handleBalance = async (ctx) => {
        ...
        const lang = await getUserLanguage(ctx.from?.id) || 'en';
        const loadingTxt = lang === 'bn' ? '⏳ *লাইভ ব্যালেন্স চেক করা হচ্ছে...*' : '⏳ *Fetching premium multi-chain portfolio...*';
        ...
        try {
            ...
            let header = lang === 'bn' ? '💰 *মাল্টি-চেইন পোর্টফোলিও ব্যালেন্স*' : '💰 *Multi-Chain Portfolio Balance*';
            let report = `${header}\n▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬\n\n`;
            ...
            for (const wallet of wallets) {
                ...
                // Get networks and sort to show main networks first
                const sortedNetworks = Object.values(NETWORKS).sort((a, b) => (a.testnet === b.testnet) ? 0 : a.testnet ? 1 : -1);
                ...
                for (const network of sortedNetworks) {
                    try {
                        ...
                        let usdtAmount = 0;
                        try {
                            const usdtBalance = await withTimeout(usdtContract.balanceOf(wallet.address), 3000);
                            let decimals = 6;
                            try { decimals = await withTimeout(usdtContract.decimals(), 2000); } catch (e) { }
                ...
                if (walletTestnetInfo) {
                    report += lang === 'bn' ? `🧪 *টেস্টনেট এসেট:*\n${walletTestnetInfo}\n` : `🧪 *Testnet Assets:*\n${walletTestnetInfo}\n`;
                }
                report += `────────────────────\n`;
            }
...
*Mandatory Channel Join:* You must be a member of our channels to access bot features.
    ...
    const handleDevInfo = async (ctx) => {
        ...
        const back = await getBackButton(ctx);
        const menu = Markup.inlineKeyboard([
            [Markup.button.url('📩 Contact Developer', `tg://user?id=${devId}`)],
            ...back.reply_markup.inline_keyboard
        ]);

        // 1. If we are currently fetching, don't start another request (spam protection)
        if (devCache.isFetching) {
            return ctx.reply(msg, { parse_mode: 'Markdown', ...menu }).catch(() => { });
        }
        ...
        if (now - devCache.lastFetch < CACHE_TTL && devCache.lastFetch !== 0) {
            if (devCache.photoId) {
                try {
                    return await ctx.replyWithPhoto(devCache.photoId, { caption: msg, parse_mode: 'Markdown', ...menu });
                } catch (err) {
                    console.error('Stale photo cache detected, falling back to text:', err.message);
                    devCache.photoId = null; // Invalidate stale photo ID
                }
            }
            return ctx.reply(msg, { parse_mode: 'Markdown', ...menu }).catch(() => { });
        }
        ...
        try {
            ...
            if (devInfo) {
                devCache.name = (devInfo.first_name || 'Admin') + (devInfo.last_name ? ' ' + devInfo.last_name : '');
            ...
            try {
                ...
                if (photos && photos.total_count > 0) {
                    ...
                    devCache.photoId = null;
                }
            } catch (pErr) {
                console.warn('Could not fetch dev photos:', pErr.message);
                devCache.photoId = null;
            }
        ...
        if (devCache.photoId) {
            try {
                await ctx.replyWithPhoto(devCache.photoId, { caption: msg, parse_mode: 'Markdown', ...menu });
            } catch (err) {
                console.error('Failed to reply with photo after fetch:', err.message);
                await ctx.reply(msg, { parse_mode: 'Markdown', ...menu }).catch(() => { });
            }
        } else {
            await ctx.reply(msg, { parse_mode: 'Markdown', ...menu }).catch(() => { });
        }
    ...
    bot.catch((err, ctx) => {
        ...
        try {
            const lang = userLanguageCache.get(ctx.from?.id) || 'en';
            ctx.reply(lang === 'bn' ? '⚠️ বোট এ একটি ত্রুটি ঘটেছে। অনুগ্রহ করে পরে চেষ্টা করুন বা ডেভেলপারকে জানান।' : '⚠️ An unexpected error occurred. Please try again later or contact the developer.').catch(() => { });
        } catch (e) { }
    });
...
async function sendTransactionAlert(bot, chatId, txData, getUserLanguage) {
    ...
    const isReceived = txData.type.toLowerCase().includes('received');
    const typeEmoji = isReceived ? '📥' : '📤';
    const statusEmoji = isReceived ? '🟢' : '🔴';
    const networkEmoji = '✨';

    const title = t('tx_alert_title', txData.network);
    const explorerBtn = t('view_explorer_label');

    const msg = `
${networkEmoji} ${title} ${networkEmoji}
▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬ ▬

👤 *Wallet:* ${txData.walletName}
💰 *Value:* $${txData.usdValue}
${statusEmoji} *Type:* ${txData.type}
💎 *Amount:* ${txData.amount} ${txData.symbol}

🔗 [View on Explorer](${txData.explorerUrl})
    `.trim();

    const bannerPath = './assets/tx_alert.png';
    const fs = require('fs');
    if (fs.existsSync(bannerPath)) {
